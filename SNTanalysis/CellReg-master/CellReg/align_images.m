...
typical_cell_size=30; % in micrometers - determines the radius that is used for gaussfit
...
if strcmp(alignment_type,'Translations and Rotations') % if correcting for rotations as well
    ...
    footprints_projections_rotated = cell(1,number_of_sessions);
    ...
    footprints_projections_rotated{reference_session_index} = footprints_projections_corrected{reference_session_index};
end

%Added by Grace to allow for another input to be used for correlation
%calculations
if numel(varargin) > 1
    im = varargin{2};
else
    im = footprints_projections;
end
...
for n=1:number_of_sessions-1
    ...
    if strcmp(alignment_type,'Translations and Rotations')
        if use_parallel_processing %changed use of centroid projections to footprint projections for cross correlation
            ...
            for r=1:length(possible_rotations)
                ...
                rotated_image=rotate_image_interp(im{registration_order(n)},possible_rotations(r),[0 0],center_of_FOV); %changed to footprint projections from centroid projections
                cross_corr=normxcorr2(im{reference_session_index},rotated_image); %changed to footprint projections from centroid projections
                temp_correlations_vector(r)=max(max(cross_corr));
            end            
            % finding the best rotation with a gaussian fit:
            [~,ind_best_rotation]=max(temp_correlations_vector);
        ...
        footprints_projections_rotated{registration_order(n)} = rotate_image_interp(footprints_projections_corrected{registration_order(n)},best_rotation,[0 0],center_of_FOV);
    end
    
    % Finding translations with subpixel resolution:
    if strcmp(alignment_type,'Translations and Rotations')
        cross_corr_cent=normxcorr2(im{reference_session_index},im{registration_order(n)}); %changed from centroid projections
    else
        cross_corr_cent=normxcorr2(im{reference_session_index},im{registration_order(n)}); %changed from centroid projections
    end
