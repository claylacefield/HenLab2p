...
lick = d(Y == 1,2);  % this gives you all the positions in column 2 rows that has licks: go fectch me every item of d(:,2)--> Y==1 replaces :
...
bins = linspace(min(d(:, 2)), max(d(:, 2)), 51);  % gives you the histogram of licks binning 51 (or you can change)
h = histc(lick, bins);
figure; bar(bins, h)     
title(behaviorCell{i}.fileName)
end

%count=histc(lick,[1000 1500])