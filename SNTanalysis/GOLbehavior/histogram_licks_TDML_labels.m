...
%     Y(ind)=1; % puts one for all the rows in column Y as indexed from 'ind' from dsearchn
    ...
    T = treadBehStruc{i}.BeCell(:, 1)';
    pos = treadBehStruc{i}.BeCell(:, 2)';
    ...
    for trialNum = 1:max(lapVec1)  %count for all the laps sequentially
        ...
        bins = linspace(min(d(:, 2)), max(d(:, 2)), 100);  % gives you a linearsapce btwn max and min and number of bins inside
...
save('treadBehStruc.mat', 'treadBehStruc');
%save treadBehStruc.mat