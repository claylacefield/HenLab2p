...
for i = 1 : length(treadBehStruc) %iterate over sessions
    ...
    licksByLap = []; %initialize the variable for all of the licks per lap
    for trialNum = 1:max(lapVec1)  %count for all the laps sequentially
        ...
        bins = linspace(min(d(:, 2)), max(d(:, 2)), 101);  % gives you a linearsapce btwn max and min and number of bins inside
       ...
       % -- now need to write a logical vector for RewLicksbyLap
       % and logical vector for PreRewLicksbyLap
