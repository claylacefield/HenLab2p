...
csvNames = dir('*csv');
licksByPosLap = {};
for i = 1 : length(csvNames) % either number of cell arrays or 1:length(csvNames) for entire array
    ...
    for trialNum = 1:max(lapVec1)  %count for all the laps sequentially
        logicalVec = Y == 1 & lapVec1 == trialNum;  % & operator is used so time has to be licktime AND lap vector has to be equal to trial number
