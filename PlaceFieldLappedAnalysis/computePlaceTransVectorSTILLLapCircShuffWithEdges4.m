function out = computePlaceTransVectorSTILLLapCircShuffWithEdges4(activity, treadPos, T, lapVec, shuffN, varargin)
%function out = computePlaceTransVectorSTILLLapCircShuffWithEdges4(activity, treadPos, T, lapVec, shuffN, excludeVec(opt))
...
trimRunStarts = 0.25;
trimRunEnds = 0.25;
minRunTime = 2; %only still for 2sec or run for 2 sec
minPFBins = 5;
minVel = -3; %if -5 calculate vel<5 cm/s
...
if shuffN > 1
    ...
    ratesR = out.posRates;
    %these concatanations are to account for the stuff around the edges
