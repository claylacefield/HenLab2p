...
if length(treadBehStruc.adjFrTimes)>maxFr*1.5
    tempDs = 2;
else
    tempDs = 1;
end
...
treadPos = treadBehStruc.resampY(1:tempDs:end);
...
for i = 1:3
rgb(:,:,i) = tiff;
a2(:,:,i) = zeros(handles.d1,handles.d2);
end
a2(:,:,2) = aSeg;
a3 = uint8(a2/max(a2(:))*255);
rgb2 = rgb + a3;
imshow(rgb2, 'Parent', handles.spatialAxes);
