function [goodSeg, goodSegParams] = findGoodSegsDG_CNMF(segStruc)

%% USAGE: [goodSeg, goodSegParams] = findGoodSegsDG_CNMF(segStruc);
% Clay 12/27/2016
...
for segNum = 1:size(C,2)
   ...
   tempThresh = 2; %tempThreshSD*std(segTemp)+mean(segTemp);
   
   bw = 20; %imbinarize(reshape(segSpat, d1,d2));  % binary image (for #pix in factor)
