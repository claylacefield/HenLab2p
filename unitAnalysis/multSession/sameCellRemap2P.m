function [remapStruc] = sameCellRemap2P(sameCellTuningStruc);
...
regMapInd = sameCellTuningStruc.regMapInd; % orig C/A ind for all cells in ziv mat
...
for i = 1:length(multSessSegStruc)
    ...
    remapStruc.fracPCs(i) = remapStruc.numPCs(i)/length(multSessSegStruc(i).pksCell);
end
...
for i = 1:size(placeCellAllInd,1)
    for j = 1:length(multSessSegStruc)
        %placeCellAllInd(i,j) = find(multSessSegStruc(j).goodSeg == placeCellAllInd(i,j));
        posRatesCell{i,j} = multSessSegStruc(j).PCLapSess.posRates(placeCellAllInd(i,j),:);
    end
end
...
for i = 1:size(placeCellAnyInd,1)
    for j = 1:length(multSessSegStruc)
        %placeCellAnyGoodSegInd(i,j) = find(multSessSegStruc(j).goodSeg == placeCellOrigInd(i,j));
        posRatesCellAnyPC{i,j} = multSessSegStruc(j).PCLapSess.posRates(placeCellAnyInd(i,j),:);
    end
end
