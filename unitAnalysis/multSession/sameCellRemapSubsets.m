function [remapStrucSubsets] = sameCellRemapSubsets(sameCellTuningStruc);
...
remapStrucSubsets.sessComb = sessComb;
...
for i = 1:size(posRatesCellAnyRew,1)
    ...
    [remapStrucSubsets.rewInAnyCoef{i}, remapStrucSubsets.rewInAnyPval{i}] = corrcoef(rates');
end

A1A2=[]; A1B=[]; A2B=[];
for i = 1:length(remapStrucSubsets.rewInAnyCoef)
    A1A2 = [A1A2; remapStrucSubsets.rewInAnyCoef{i}(2)];
    A1B = [A1B; remapStrucSubsets.rewInAnyCoef{i}(3)];
    A2B = [A2B; remapStrucSubsets.rewInAnyCoef{i}(6)];
end
remapStrucSubsets.RewAnyCorrCoeff121323 = [A1A2, A1B, A2B];
...
remapStrucSubsets.RewMatchedAny = [RewMatchedAny];
...
for i = 1:size(posRatesCell,1)
    ...
    [remapStrucSubsets.allZivCorrCoef{i}, remapStrucSubsets.allZivCorrPval{i}] = corrcoef(rates');
end


%% now use this to compare correlations bet. 1-2, 2-3
% 
firstTwoPcell = remapStrucSubsets.allZivCorrPval(remapStrucSubsets.sessComb.firstTwoOnly);
firstTwoCorrCell = remapStrucSubsets.allZivCorrCoef(remapStrucSubsets.sessComb.firstTwoOnly);
for i = 1:length(firstTwoPcell)
    firstTwoP(i)=firstTwoPcell{i}(2);
    firstTwoCorr(i)=firstTwoCorrCell{i}(2);
end

lastTwoPcell = remapStrucSubsets.allZivCorrPval(remapStrucSubsets.sessComb.lastTwoOnly);
lastTwoCorrCell = remapStrucSubsets.allZivCorrCoef(remapStrucSubsets.sessComb.lastTwoOnly);
