...
[evTrigSig1, zeroFr] = eventTrigSig(C(segNum,:), evTimes(pos1evInd), 0, [-30 100], frTimes(1:2:end));
[evTrigSig2, zeroFr] = eventTrigSig(C(segNum,:), evTimes(pos2evInd), 0, [-30 100], frTimes(1:2:end));
else
...
figure;
try
plotMeanSEMshaderr(evTrigSig0, 'r',30);
catch
end
hold on;
plotMeanSEMshaderr(evTrigSig1, 'g',30);
try
    plotMeanSEMshaderr(evTrigSig2, 'b',30);
catch
end
