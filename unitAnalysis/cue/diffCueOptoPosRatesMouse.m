function [diffOptoPosRatesStruc] = diffCueOptoPosRatesMouse()
...
for j=3:length(mouseDir)    % go through all days
    ...
    try
        ...
        for i = 3:length(dayDir) % and all sessions
            try
                if (~isempty(strfind(dayDir(i).name, '20')) || ~isempty(strfind(dayDir(i).name, '19'))) && contains(dayDir(i).name, 'cueOmitOlfOpto-') && ~contains(dayDir(i).name, 'Dk11')
...
%                     end
                    ...
                    %if contains(dayDir(i).name, 'mit') %&& ~isempty(find(lapTypeArr==0)) || contains(dayDir(i).name, 'hift') || 
                        ...
                        toPlot=0; 
                        [dPFratePCsess, pkPosPCsess, relPFratePCsess] = diffCueOptoPosRates(cueShiftStruc, toPlot);
...
diffOptoPosRatesStruc.pathCell = pathCell;
diffOptoPosRatesStruc.cueShiftNameCell = cueShiftNameCell;
diffOptoPosRatesStruc.pcCell = pcCell;
diffOptoPosRatesStruc.dPFratePC = dPFratePC;
diffOptoPosRatesStruc.dPFrateCell = dPFrateCell;
diffOptoPosRatesStruc.relPFratePC = relPFratePC;
diffOptoPosRatesStruc.relPFrateCell = relPFrateCell;
diffOptoPosRatesStruc.pkPosPC = pkPosPC;
diffOptoPosRatesStruc.pkPosCell = pkPosCell;


%% save and plot
try
    
save(['diffOptoPosRatesStruc_' date '.mat'], 'diffOptoPosRatesStruc');

plotDiffOmitPosRatesStruc(diffOptoPosRatesStruc);
