function [posBinFrac, posInfo, pcRatesBlanked, pcOmitRatesBlanked, pcOptoRatesBlanked, pcOmitOptoRatesBlanked, pfOnlyRates, pfOnlyRatesOmit, pfOnlyRatesOpto,  pfOnlyRatesOmitOpto, pcRatesSorted, omitRatesSorted, optoRatesSorted, omitOptoRatesSorted] = cuePosInhibOpto(cueShiftStruc, goodSeg, refLapType, toPlot);
...
posRates3 = cueShiftStruc.PCLappedSessCell{3}.posRates; % for Omit laps
pcRates3 = posRates3(pc,:);
optoRatesSorted = pcRates3(sortInds,:);
posRates4 = cueShiftStruc.PCLappedSessCell{2}.posRates; % for Omit laps
pcRates4 = posRates4(pc,:);
omitOptoRatesSorted = pcRates4(sortInds,:);
...
for i = 1:size(pcRatesSorted,1) % for all cells (actually not just pc now)
    ...
    rates3 = optoRatesSorted(i,:);
    rates4 = omitOptoRatesSorted(i,:);
    pfRates = pcRatesSorted(i,:);
    pfRates2 = omitRatesSorted(i,:);
    pfRates3 = optoRatesSorted(i,:);
    pfRates4 = omitOptoRatesSorted(i,:);
    ...
    if pkPos <= 10 % wraparound for units at beginning
        ...
        rates4(1:pkPos+9) = NaN; % rates2(1:pkPos+10) = NaN;
        rates4(100-(9-pkPos):100) = NaN; % rates2(100-10-pkPos:100) = NaN;
        ...
        pfRates4(pkPos+10:100-(10-pkPos)) = NaN;
    ...
    elseif pkPos>90 % wraparound for units at end
        ...
        pfRates4(1:pkPos-10) = NaN;
        pfRates4(pkPos+10:end) = NaN;
        ...
        if pkPos>45 && pkPos<55 % for potential middle-cue cells (PF in pos 40-60)
            ...
            optoPosRate(j) = mean(optoRatesSorted(i,45:55),2);
            omitOptoPosRate(j) = mean(optoRatesSorted(i,45:55),2);
            cuePkPos(j) = pkPos;
        end
    ...
    pcOptoRatesBlanked(i,:) = rates3;
    pcOmitOptoRatesBlanked(i,:) = rates4;
    pfOnlyRates(i,:) = pfRates;
    pfOnlyRatesOmit(i,:) = pfRates2;   
    pfOnlyRatesOpto(i,:) = pfRates3; 
    pfOnlyRatesOmitOpto(i,:) = pfRates4; 
end
...
subplot(2,3,3);
imagesc(pcOptoRatesBlanked);
title('pcOptoRatesBlanked');
subplot(2,3,4);
plot(nanmean(pcRatesBlanked,1));
hold on;
plot(nanmean(pcOmitRatesBlanked,1),'r');
plot(nanmean(pcOptoRatesBlanked,1),'g');
