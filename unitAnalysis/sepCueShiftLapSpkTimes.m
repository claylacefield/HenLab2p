function [pksCell1, posLap1, pksCell2, posLap2] = sepCueShiftLapSpkTimes(pksCell, goodSeg, treadBehStruc)
...
for i=1:length(lapFrInds)-1
    ...
    for j=1:length(pksCellGood)
        ...
        if lapType==1
            pksCell1{j} = [pksCell1{j} lapPks'];
        else
            pksCell2{j} = [pksCell2{j} lapPks'];
        end
    end
