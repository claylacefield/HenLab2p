...
placeCellInNoneOrigInd = sameCellTuningStruc.placeCellInNoneOrigInd ;
placeCellInAnyOrigInd = sameCellTuningStruc.placeCellInAnyOrigInd;
...
for i = 1:size(posRatesCellAllPC, 2)
    PCMatchedAll{i} = [];
    for ii = 1:size(posRatesCellAllPC, 1)
        PCMatchedAll{i} = [PCMatchedAll{i}; posRatesCellAllPC{ii, i}];
    end
end
