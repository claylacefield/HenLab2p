...
while stillAdding
    try
        [segDictName, path] = uigetfile('*.mat', 'Select session seg2P file to add to multSessTuningStruc');
        cd(path);
...
%             [goodSegPosPkStruc, circStatStruc] = wrapUnitTuning(C, treadBehStruc, numBins, rayThresh);
            ...
            multSessSegStruc(numSess).C = seg2P.C2p;
            multSessSegStruc(numSess).A = seg2P.A2p;
            multSessSegStruc(numSess).d1 = seg2P.d12p;
            multSessSegStruc(numSess).d2 = seg2P.d22p;
...
%             end
            ...
            multSessSegStruc(numSess).pksCell = seg2P.pksCell;
...
%             multSessTuningStruc(numSess).placeCellStruc.goodRay = wellTunedInd;
        ...
        saveFilename = ['/Backup20TB/Sebnem/' multSessSegStruc(1).mouseName '/' multSessSegStruc(1).dayName '/' multSessSegStruc(1).mouseName '_' multSessSegStruc(1).dayName '_multSessSegStruc_' date '.mat'];
