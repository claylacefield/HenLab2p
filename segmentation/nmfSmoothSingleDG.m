function [segStruc] = nmfSmoothSingleDG(K,beta, toDownsample) %plotSeg, toSmooth, toSave)
...
inStruc = load(nam);
Y = inStruc.(char(fieldnames(inStruc))); % only works if one var
...
%Yav = mean(Y,3); avmat = repmat(mean(Y,3), [1 1 T]);
%Y2 = Y-Yav; 
